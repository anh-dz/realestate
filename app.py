...
from data_manager import get_db_connection, get_properties_data, delete_property
import datetime
...
def suggest():
    ...
    if request.method == 'POST':
        try:
            ...
            sql = """
                SELECT p.property_id, p.address, t.price_per_sqm, d.district_name, b.building_type,
                ...
                ORDER BY t.price_per_sqm DESC LIMIT 50
            """
...
def add():
    ...
    if request.method == 'POST':
        try:
            ...
            if parking_type:
                cursor.execute("INSERT INTO Parking (property_id, parking_type, parking_price) VALUES (?, ?, ?)", (property_id, parking_type, parking_price))
...
def edit(id):
    ...
    if request.method == 'POST':
        try:
            ...
            conn.rollback()
            print(f"Error editing property: {e}")
